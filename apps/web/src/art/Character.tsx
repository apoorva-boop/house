  ...
  return (
    <svg viewBox="0 0 80 120" width="100%" height="100%" role="img" aria-hidden="true">
      <g transform={`rotate(${pose.lean} 40 70)`}>
        ...
        {/* Legs */}
        <rect x={28} y={82} width={10} height={30} rx={3} fill={shade(PANTS, "mid")} />
        <rect x={42} y={82} width={10} height={30} rx={3} fill={shade(PANTS, "dark")} />
        ...
        {/* Head */}
        <circle cx={40} cy={24} r={14} fill={tone(SKIN)} />

        {/* Brows: level and neutral when fresh, angled down toward the nose when worn out */}
        <line x1={30} y1={eyeY - 5} x2={36} y2={eyeY - 5 + pose.browTilt * 0.12} stroke={tone("character-brow")} strokeWidth={1.6} strokeLinecap="round" />
        <line x1={50} y1={eyeY - 5} x2={44} y2={eyeY - 5 + pose.browTilt * 0.12} stroke={tone("character-brow")} strokeWidth={1.6} strokeLinecap="round" />

        {/* Eyes: open circles when fresh, half-closed lines once tiredness passes the midpoint */}
        {pose.eyesOpen ? (
          <>
            <circle cx={35} cy={eyeY} r={1.8} fill={tone("character-eye")} />
            <circle cx={45} cy={eyeY} r={1.8} fill={tone("character-eye")} />
          </>
        ) : (
          <>
            <line x1={33} y1={eyeY} x2={37} y2={eyeY} stroke={tone("character-eye")} strokeWidth={1.6} strokeLinecap="round" />
            <line x1={43} y1={eyeY} x2={47} y2={eyeY} stroke={tone("character-eye")} strokeWidth={1.6} strokeLinecap="round" />
          </>
        )}
        ...
        <path
          ...
          fill="none"
          stroke={tone("character-mouth")}
          strokeWidth={1.6}
          strokeLinecap="round"
        />

        {/* A bead of sweat once exhaustion sets in, tiers 0.6 and 0.8 only */}
        {pose.sweat && <path d="M 54 18 q 3 4 0 7 q -3 -1 -3 -3.5 q 0 -2.5 3 -3.5 Z" fill={tone("character-sweat")} />}
      </g>
    </svg>
  );
}
